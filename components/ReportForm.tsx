...
const ReportForm: React.FC<ReportFormProps> = ({ onCancel, onSubmit }) => {
      ...
      <div className="p-8 md:p-12">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {Object.values(ViolenceType).map(type => (
                  <button
                    ...
                    className={`p-3 rounded-xl text-[10px] font-bold text-center border-2 transition-all ${formData.facts.violenceTypes.includes(type) ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-slate-100 text-slate-500 hover:border-slate-200'}`}
                  >
                    {type}
                  </button>
                ))}
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                <div className="p-12 border-4 border-dashed border-slate-100 rounded-[2.5rem] flex flex-col items-center justify-center text-center gap-4 hover:border-amber-200 transition-colors group cursor-pointer">
        ...
        <div className="flex items-center justify-between mt-12 pt-8 border-t border-slate-100">
          ...
          <button
            ...
            className={`px-12 py-5 rounded-[2rem] font-black text-xs uppercase tracking-widest transition-all ${isStepValid() ? 'bg-amber-500 text-white shadow-xl shadow-amber-100 hover:-translate-y-1 hover:bg-amber-600' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}
          >
            {step === 5 ? 'Enviar ara' : 'Següent pas'}
          </button>
        </div>
      </div>
    </div>
